...
exports.insertData = (req, res, next) => {
  ...
  for (let index = 0; index < 5697; index++) {
    fq.readFile(`${path}.json`, function (err, data) {
      ...
      const { name, description, external_url, image, attributes } =
        JSON.parse(data);

      const flag = attributes.some((data) => data.trait_type === "BODY");
      if (flag) {
        let BACKGROUND = attributes.find(
          (data) => data.trait_type === "BACKGROUND"
        ).value;
        let HOOD = attributes.find((data) => data.trait_type === "HOOD").value;
        let NEONGLOW = attributes.find(
          (data) => data.trait_type === "NEON GLOW"
        ).value;
        let DOOR = attributes.find((data) => data.trait_type === "DOOR").value;
        let COLOR = attributes.find(
          (data) => data.trait_type === "COLOR"
        ).value;
        let RIMS = attributes.find((data) => data.trait_type === "RIMS").value;
        let REAREXHAUST = attributes.find(
          (data) => data.trait_type === "REAR EXHAUST"
        ).value;
        let SIDEEXHAUST = attributes.find(
          (data) => data.trait_type === "SIDE EXHAUST"
        ).value;
        let HEADLIGHTS = attributes.find(
          (data) => data.trait_type === "HEADLIGHTS"
        ).value;
        let BUMPER = attributes.find(
          (data) => data.trait_type === "BUMPER"
        ).value;
        let ROOF = attributes.find((data) => data.trait_type === "ROOF").value;
        let BODY = attributes.find((data) => data.trait_type === "BODY").value;
        let TOPSPEED = attributes.find(
          (data) => data.trait_type === "TOP SPEED"
        );
        let ACCELERATION = attributes.find(
          (data) => data.trait_type === "ACCELERATION"
        );
        let NITROUS = attributes.find((data) => data.trait_type === "NITROUS");
        let HANDLING = attributes.find(
          (data) => data.trait_type === "HANDLING"
        );
        ...
        let Mouth = attributes.find(
          (data) => data.trait_type === "Mouth"
        ).value;
        let Head = attributes.find((data) => data.trait_type === "Head").value;
        let Clothing = attributes.find(
          (data) => data.trait_type === "Clothing"
        ).value;
        let Weapon = attributes.find(
          (data) => data.trait_type === "Weapon"
        ).value;
        let Species = attributes.find(
          (data) => data.trait_type === "Species"
        ).value;
...
exports.getCars = async (req, res, next) => {
  try {
    ...
    const data = await Gallery.find({
      ...
      .skip(limit * (page - 1))
      .limit(limit);
    const dataLength = await Gallery.count();
    console.log(data , dataLength)
    res.status(200).json({
      cars: data,
      length: dataLength,
      flag: true,
      msg: "Succefully get Cars",
    });
  } catch (error) {
  ...
  //     .status(200)
  //     .json({
  //       cars: response.splice(0, limit * page),
...
exports.getAvatars = (req, res, next) => {
  ...
  Gallery.find({
    ...
    .then(async function (response) {
      ...
      console.log(response);
      res.status(200).json({
        avatars: response.splice(0, limit * page),
        length: length,
        flag: true,
        msg: "Succefully get Avatars",
      });
    })
    .catch((error) => {
      res.status(501).json({ error: error });
...
exports.getPost = (req, res, next) => {
  ...
  Gallery.findOne({ _id: id })
    .select(
      "-attributes.TOPSPEED -attributes.ACCELERATION -attributes.NITROUS -attributes.HANDLING -attributes.ARMOR"
    )
...
exports.searchFilterAvatars = async (req, res, next) => {
  ...
  console.log(key);
  if (key === "Background") {
    let Background = await Gallery.distinct("attributes.Background", {
  ...
  if (key === "Skin") {
    let Skin = await Gallery.distinct("attributes.Skin", {
      "attributes.Skin": { $regex: ".*" + skin + ".*", $options: "i" },
    });
    Skin = await Skin.filter((item) => item !== "");
  ...
  if (key === "Eyes") {
    let Eyes = await Gallery.distinct("attributes.Eyes", {
      "attributes.Eyes": { $regex: ".*" + eyes + ".*", $options: "i" },
    });
    Eyes = await Eyes.filter((item) => item !== "");
  ...
  if (key === "Mouth") {
    let Mouth = await Gallery.distinct("attributes.Mouth", {
      "attributes.Mouth": { $regex: ".*" + mouth + ".*", $options: "i" },
    });
    Mouth = await Mouth.filter((item) => item !== "");
  ...
  if (key === "Head") {
    let Head = await Gallery.distinct("attributes.Head", {
      "attributes.Head": { $regex: ".*" + head + ".*", $options: "i" },
    });
    Head = await Head.filter((item) => item !== "");
  ...
  if (key === "Clothing") {
    let Clothing = await Gallery.distinct("attributes.Clothing", {
      "attributes.Clothing": { $regex: ".*" + clothing + ".*", $options: "i" },
    });
    Clothing = await Clothing.filter((item) => item !== "");
  ...
  if (key === "Weapon") {
    let Weapon = await Gallery.distinct("attributes.Weapon", {
      "attributes.Weapon": { $regex: ".*" + weapon + ".*", $options: "i" },
    });
    Weapon = await Weapon.filter((item) => item !== "");
  ...
  if (key === "Species") {
    let Species = await Gallery.distinct("attributes.Species", {
      "attributes.Species": { $regex: ".*" + species + ".*", $options: "i" },
    });
    Species = await Species.filter((item) => item !== "");
...
exports.searchFilterCars = async (req, res, next) => {
  ...
  if (hood.length > 0) {
    let HOOD = await Gallery.distinct("attributes.HOOD", {
      "attributes.HOOD": { $regex: ".*" + hood + ".*", $options: "i" },
    });
    res.status(200).json({ HOOD: HOOD });
    return;
  }
  if (neonglow.length > 0) {
    let NEONGLOW = await Gallery.distinct("attributes.NEONGLOW", {
      "attributes.NEONGLOW": { $regex: ".*" + neonglow + ".*", $options: "i" },
    });
    res.status(200).json({ NEONGLOW: NEONGLOW });
    return;
  }
  if (door.length > 0) {
    let DOOR = await Gallery.distinct("attributes.DOOR", {
      "attributes.DOOR": { $regex: ".*" + door + ".*", $options: "i" },
    });
    res.status(200).json({ DOOR: DOOR });
    return;
  }
  if (color.length > 0) {
    let COLOR = await Gallery.distinct("attributes.COLOR", {
  ...
  if (rims.length > 0) {
    let RIMS = await Gallery.distinct("attributes.RIMS", {
      "attributes.RIMS": { $regex: ".*" + rims + ".*", $options: "i" },
    });
  ...
  if (rearexhaust.length > 0) {
    let REAREXHAUST = await Gallery.distinct("attributes.REAREXHAUST", {
      "attributes.REAREXHAUST": {
        $regex: ".*" + rearexhaust + ".*",
        $options: "i",
      },
    });
  ...
  if (sideexhaust.length > 0) {
    let SIDEEXHAUST = await Gallery.distinct("attributes.SIDEEXHAUST", {
      "attributes.SIDEEXHAUST": {
        $regex: ".*" + sideexhaust + ".*",
        $options: "i",
      },
    });
  ...
  if (headlights.length > 0) {
    let HEADLIGHTS = await Gallery.distinct("attributes.HEADLIGHTS", {
      "attributes.HEADLIGHTS": {
        $regex: ".*" + headlights + ".*",
        $options: "i",
      },
    });
  ...
  if (bumper.length > 0) {
    let BUMPER = await Gallery.distinct("attributes.BUMPER", {
      "attributes.BUMPER": { $regex: ".*" + bumper + ".*", $options: "i" },
    });
    res.status(200).json({ BUMPER: BUMPER });
    return;
  }
  if (roof.length > 0) {
    let ROOF = await Gallery.distinct("attributes.ROOF", {
      "attributes.ROOF": { $regex: ".*" + roof + ".*", $options: "i" },
    });
    res.status(200).json({ ROOF: ROOF });
    return;
  }
  if (body.length > 0) {
    let BODY = await Gallery.distinct("attributes.BODY", {
      "attributes.BODY": { $regex: ".*" + body + ".*", $options: "i" },
    });
    ...
    const cars = Gallery.find({
      ...
      .then(async (cars) => {
        ...
        await res.status(200).json({
          msg: "Successfully Fetched Cars",
          cars: cars.splice(0, limit),
          length: length,
          flag: true,
        });
      })
...
exports.nameSearchAvatars = async (req, res, next) => {
  ...
  if (search.length >= 1 || !avatars) {
    const avatars = await Gallery.find({
      name: { $regex: ".*" + search + ".*", $options: "i" },
      "attributes.HOOD": { $exists: false },
    })
      .exec()
    ...
    await res.status(200).json({
      msg: "Successfully Fetched Avatars",
      avatars: avatars.splice(0, limit),
      length: length,
      flag: true,
    });
